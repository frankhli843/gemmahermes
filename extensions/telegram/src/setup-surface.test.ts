...
describe("telegram DM access warning helpers", () => {
  it("shows global config commands for the default account", () => {
    ...
    expect(lines.join("\n")).toContain(
      'gemmahermes config set channels.telegram.dmPolicy "allowlist"',
    );
    expect(lines.join("\n")).toContain(
      `gemmahermes config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`,
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", () => {
    ...
    expect(lines.join("\n")).toContain(
      'gemmahermes config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
    );
    expect(lines.join("\n")).toContain(
      `gemmahermes config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
    );
  });
