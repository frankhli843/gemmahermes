# GemmaHermes ACP Bridge

This document describes how the GemmaHermes ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`gemmahermes acp` exposes an ACP agent over stdio and forwards prompts to a running
GemmaHermes Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
`gemmahermes acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It is designed to route IDE prompts into an existing GemmaHermes Gateway
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the GemmaHermes gateway or agent instead.                                                                                                                                    |
...
drive a GemmaHermes Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `gemmahermes acp` over stdio.

Example config:

```bash
gemmahermes config set gateway.remote.url wss://gateway-host:18789
gemmahermes config set gateway.remote.token <token>
```

Example run:

```bash
gemmahermes acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
gemmahermes acp --session agent:main:main
gemmahermes acp --session agent:design:main
gemmahermes acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “GemmaHermes ACP” to start a thread.

## Execution Model

- ACP client spawns `gemmahermes acp` and speaks ACP messages over stdio.
...
```bash
gemmahermes acp --session agent:main:main
gemmahermes acp --session-label "support inbox"
gemmahermes acp --reset-session
```
...
`gemmahermes acp` resolves the Gateway URL and auth from CLI flags or config:
