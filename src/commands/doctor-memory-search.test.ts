...
describe("noteMemorySearchHealth", () => {
  ...
  it("warns when lmstudio gateway probe is unavailable", async () => {
    ...
    expect(message).toContain("gemmahermes memory status --deep");
  });
  ...
  it("uses model configure hint when gateway probe is unavailable and API key is missing", async () => {
    ...
    expect(message).toContain("gemmahermes configure --section model");
    expect(message).not.toContain("gemmahermes auth add --provider");
  });
