...
describe("nodes camera helpers", () => {
  ...
  it("builds stable temp paths when id provided", () => {
    ...
    expect(p).toBe(path.join("/tmp", "gemmahermes-camera-snap-front-id1.jpg"));
  });
  ...
  it("writes camera clip payload to temp path", async () => {
    await withCameraTempDir(async (dir) => {
      ...
      expect(out).toBe(path.join(dir, "gemmahermes-camera-clip-front-clip1.mp4"));
  ...
  it("writes camera clip payload from url", async () => {
    ...
    await withCameraTempDir(async (dir) => {
      ...
      expect(out).toBe(path.join(dir, "gemmahermes-camera-clip-back-clip2.mp4"));
