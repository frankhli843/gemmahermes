...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+(?:gemmahermes|openclaw)\b|^(?:gemmahermes|openclaw)\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+(?:gemmahermes|openclaw)\b.*(?:^|\s)update(?:\s|$)|^(?:gemmahermes|openclaw)\b.*(?:^|\s)update(?:\s|$)/;
