...
- In scope: strictly user-supplied credentials that GemmaHermes does not mint or rotate.
...
- Marker persistence is source-authoritative: GemmaHermes writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
