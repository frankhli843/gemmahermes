...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. GemmaHermes normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, GemmaHermes seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so GemmaHermes can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. GemmaHermes does not need to inject block-level cache markers.
- GemmaHermes uses `prompt_cache_key` to keep cache routing stable across turns and uses `prompt_cache_retention: "24h"` only when `cacheRetention: "long"` is selected on direct OpenAI hosts.
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). GemmaHermes maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, GemmaHermes injects Anthropic
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, GemmaHermes
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; GemmaHermes maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, GemmaHermes automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  GemmaHermes manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  GemmaHermes maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, GemmaHermes derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean GemmaHermes is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

GemmaHermes splits the system prompt into a **stable prefix** and a **volatile
...
## GemmaHermes cache-stability guards

GemmaHermes also keeps several cache-sensitive payload shapes deterministic before
...
GemmaHermes exposes dedicated cache-trace diagnostics for embedded agent runs.
...
GemmaHermes keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. GemmaHermes's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
