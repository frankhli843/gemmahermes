...
This document explains how GemmaHermes manages sessions end-to-end:
...
GemmaHermes is designed around a single **Gateway process** that owns session state.
...
GemmaHermes persists sessions in two layers:
...
GemmaHermes resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, GemmaHermes reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
gemmahermes sessions cleanup --dry-run
gemmahermes sessions cleanup --enforce
```
...
GemmaHermes intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When GemmaHermes splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, GemmaHermes
...
- If a trailing tool-result block would otherwise push the chunk over target,
  GemmaHermes preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (GemmaHermes consumes the events, but Pi decides when to compact).
...
GemmaHermes also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, GemmaHermes bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, GemmaHermes leaves it alone.
...
- If the provider fails or returns an empty result, GemmaHermes falls back to built-in LLM summarization automatically.
...
- `gemmahermes status` (CLI)
- `gemmahermes sessions` / `sessions --json`
...
GemmaHermes supports “silent” turns for background tasks where the user should not see intermediate output.
...
- GemmaHermes strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, GemmaHermes also suppresses **draft/typing streaming** when a
...
GemmaHermes uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but GemmaHermes’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `gemmahermes status`.
