...
If you believe you've found a security issue in GemmaHermes, please report it privately.
For issues in the upstream OpenClaw core, report to the upstream project directly.
...
- Tested version details (GemmaHermes/OpenClaw version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through GemmaHermes. Showing that GemmaHermes can reach a native parser is not enough by itself.
- Demonstrated impact tied to GemmaHermes's (or upstream OpenClaw's) documented trust boundaries.
- For exposed-secret reports: proof the credential is GemmaHermes/OpenClaw-owned (or grants access to GemmaHermes/OpenClaw-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example Sharp/libvips/libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through GemmaHermes. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
...
GemmaHermes is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
GemmaHermes does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- GemmaHermes can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need GemmaHermes, use one VPS (or host/OS user boundary) per user.
...
- This is expected in GemmaHermes's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of GemmaHermes's trusted computing base for a gateway.
...
- Using GemmaHermes in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped GemmaHermes production code or a published package artifact intended for users.
...
- Exposed secrets that are third-party/user-controlled credentials (not GemmaHermes-owned and not granting access to GemmaHermes-operated infrastructure/services) without demonstrated GemmaHermes impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

GemmaHermes security guidance assumes:

- The host where GemmaHermes runs is within a trusted OS/admin boundary.
...
GemmaHermes's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
GemmaHermes separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when GemmaHermes can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the GemmaHermes process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

GemmaHermes uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
- Sandbox media validation allows absolute temp paths only under the GemmaHermes-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use GemmaHermes temp helpers (`resolvePreferredOpenClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
For threat model + hardening guidance (including `gemmahermes security audit --deep` and `--fix`), see:
...
GemmaHermes's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `gemmahermes gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - GemmaHermes keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `gemmahermes security audit` as dangerous findings.
...
GemmaHermes requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running GemmaHermes in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v gemmahermes-data:/app/data \
  gemmahermes/gemmahermes:latest
```
